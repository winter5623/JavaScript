...
DATA.forEach((d, j) => {
  ...
  styles.col.forEach((s, i) => {
    if (s !== 'PRO') {
      ...
      let bar = line
      .append('g')
      .attr('id', `bar-${j}`).attr('transform', `translate(${styles.x[i]}, 0)`).attr('text-anchor', styles.tA[i])
...
styles.col.forEach((s, j) => {
  ...
  DATA.forEach((d, i) => {
    if (s !== 'PRO') {
      ...
      let bar = line
      .append('g')
      .attr('id', `bar-${i}`).attr('transform', `translate(0, ${y_})`).attr('text-anchor', styles.tA[i])
